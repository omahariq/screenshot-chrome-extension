...
export function drawOverlay(canvas, metadata) {
  ...
  if (metadata.flowOrSetName) {
    ...
    lines.push(`${label}: ${metadata.flowOrSetName}`);
  }
  if (metadata.stepOrIndex !== undefined) {
    const label = metadata.mode === 'flow' ? 'Step' : 'Index';
    lines.push(`${label}: ${metadata.stepOrIndex}`);
  }
  ...
  const headerHeight = lines.length * lineHeight + padding * 2 + separatorHeight;
  ...
  for (let i = 0; i < lines.length; i++) {
    // Highlight labels in accent color
    const colonIdx = lines[i].indexOf(':');
    if (colonIdx > 0) {
      const label = lines[i].substring(0, colonIdx + 1);
      const value = lines[i].substring(colonIdx + 1);
      ctx.fillStyle = OVERLAY.ACCENT_COLOR;
      ctx.fillText(label, padding, padding + i * lineHeight);
      const labelWidth = ctx.measureText(label).width;
      ctx.fillStyle = OVERLAY.TEXT_COLOR;
      ctx.fillText(value, padding + labelWidth, padding + i * lineHeight);
    } else {
      ctx.fillStyle = OVERLAY.TEXT_COLOR;
